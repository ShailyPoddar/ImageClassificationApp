...
namespace ImageClassificationApp
{
...

    {
        ...
        public ContentResult Post()
        {
            ...
            var uploadedimg = Request.Form.Files[0];
            byte[] imgBytes = ReadFully(uploadedimg.OpenReadStream());
            ...
            ClassifySingleImage(mlContext, testSet, trainedModel, imgBytes);
        ...
        //---test------
        public static void ClassifySingleImage(MLContext mlContext, IDataView data,  ITransformer trainedModel, byte[] imgBytes)
        {
            PredictionEngine<ModelInput, ModelOutput> predictionEngine = mlContext.Model.CreatePredictionEngine<ModelInput, ModelOutput>(trainedModel);

            //ModelInput image = mlContext.Data.CreateEnumerable<ModelInput>(data, reuseRowObject: true).FirstOrDefault();
            //byte[] imgData = System.IO.File.ReadAllBytes(@"C:\Users\sesa567317\source\repos\ImageClassificationApp\assets\testData\clean.png");
